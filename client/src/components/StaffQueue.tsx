        ...
        {loading ? (
          ...
          <>
            ...
            <div className="table-responsive d-none d-md-block">
              <table className="table table-hover align-middle mb-0">
                ...
                <tbody style={{ borderTop: 'none' }}>
                  {tickets.map(ticket => (
                    <tr key={ticket.id} onClick={() => (user?.role === 'IT Staff' || user?.role === 'Administrator') && onTicketClick?.(ticket.id)} style={{ cursor: (user?.role === 'IT Staff' || user?.role === 'Administrator') ? 'pointer' : 'default' }}>
            ...
            <div className="d-md-none">
              {tickets.map(ticket => (
                <div key={ticket.id} className="card mb-3 shadow-sm" onClick={() => (user?.role === 'IT Staff' || user?.role === 'Administrator') && onTicketClick?.(ticket.id)} style={{ cursor: (user?.role === 'IT Staff' || user?.role === 'Administrator') ? 'pointer' : 'default' }}>
                  <div className="card-body">
