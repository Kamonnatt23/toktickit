...
function restoreUser(email) {
  ...
  test('valid login, mandatory password change, logout, and blocked access', async ({ page, request }) => {
    ...
    const unauthRes = await request.get('http://127.0.0.1:3001/api/tickets');
    ...
    const resStatus = await page.evaluate(async () => {
      const res = await fetch('http://127.0.0.1:3001/api/staff/tickets');
      return res.status;
    });
    ...
    const postLogoutStatus = await page.evaluate(async () => {
      const res = await fetch('http://127.0.0.1:3001/api/tickets');
      return res.status;
    });
