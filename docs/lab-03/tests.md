...
| **VIS-01**  | Visual/A11y | AC-17 | Responsive tables | Staff queue collapses to cards on mobile viewport; passes a11y checks | `client/e2e/lab-03/staff-ticket-flow.spec.ts` | Planned |

## 3. Test Coverage Goals
*   **Security/Authorization:** High priority. Every endpoint must be tested with no token, an invalid token, a token for an unauthorized role, and a valid token. Non-owned resources must assert a 404 response.
