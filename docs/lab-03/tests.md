...
|---|---|---|---|---|---|---|
| **AUTH-01** | API | AC-01 | Login with valid credentials | 200 OK, Set-Cookie header present with stateful Session ID, returns user data | `server/tests/lab-03/auth.api.test.ts` | Planned |
...
| **AUTHZ-01**| API | AC-05, AC-06 | Direct API access to other's ticket (Requester) | 404 Not Found (Non-enumeration policy) | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| **AUTHZ-02**| API | AC-06 | Requester attempts to read/write Internal Note | 404 Not Found (if attached to unowned ticket) or 403 Forbidden | `server/tests/lab-03/comments-notes.api.test.ts` | Planned |
| **AUTHZ-03**| API | AC-07, AC-08, BR-03 | Administrator attempts to claim ticket or post comment/note | 403 Forbidden (Admin has READ-ONLY access to tickets/comments/notes) | `server/tests/lab-03/authorization.api.test.ts` | Planned |
...
| **STAFF-03**| API | AC-08 | Claim and Reassign ticket | Owner updates successfully to provided IT Staff ID | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
| **STAFF-04**| API | AC-09 | Status transition validation | Invalid transitions (e.g., New to Resolved directly) return 400 Bad Request | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| **COMM-02** | API | AC-12 | Requester triggers "Appears Resolved" | 201 Created, standardized comment appended, ticket status remains unchanged | `server/tests/lab-03/comments-notes.api.test.ts` | Planned |
| **COMM-03** | API | AC-10 | Requester posts comment on "Waiting for Requester" ticket | 201 Created, Ticket status automatically transitions to "Open" | `server/tests/lab-03/comments-notes.api.test.ts` | Planned |
...
| **ADMIN-03**| API | AC-15 | Last active admin deactivation | 400 Bad Request, system refuses to have 0 admins | `server/tests/lab-03/users-admin.api.test.ts` | Planned |
...
| **VIS-01**  | Visual/A11y | AC-16 | Responsive tables | Staff queue collapses to cards on mobile viewport; passes a11y checks | `client/e2e/lab-03/staff-ticket-flow.spec.ts` | Planned |

## 3. Test Coverage Goals
*   **Security/Authorization:** High priority. Every endpoint must be tested with no token, an invalid token, a token for an unauthorized role (e.g., Admin testing Staff modification routes resulting in 403), and a valid token. Non-owned resources must assert a 404 response.
