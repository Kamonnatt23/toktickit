...
*   **FR-05 [Remove Attachment]:** The system shall allow the Requester to remove uploaded attachments before finalizing the ticket submission, and soft-remove attachments from existing tickets on the Ticket Detail screen.
...
*   **BR-02 [Required Fields]:** Category, Summary, Requested Priority, and Description are mandatory fields for ticket creation.
...
*   `POST /api/attachments/upload` - Endpoint to handle file upload (both pre-ticket creation and appending to existing tickets).
...
*   **AC-06:** Given I have an active attachment on an existing ticket, When I click remove on the Ticket Detail screen, Then the attachment is no longer visible on the UI, and it is marked as soft-deleted in the database.
